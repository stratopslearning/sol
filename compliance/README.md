# Compliance index

Internal FERPA + SOC 2–style binder (no third-party audit).

| Doc | Purpose |
| --- | --- |
| [HECVAT_LITE_ANSWERS.md](./HECVAT_LITE_ANSWERS.md) | Complete standalone HECVAT Lite 3.06 vendor response (plus FERPA/AI addendum); copy into the official workbook |
| [CONTROL_MATRIX.md](./CONTROL_MATRIX.md) | Self-audit control matrix |
| [SELF_AUDIT.md](./SELF_AUDIT.md) | Type I / Type II cadence |
| [ARCHITECTURE.md](./ARCHITECTURE.md) | Data-flow diagram |
| [DATA_INVENTORY.md](./DATA_INVENTORY.md) | Education-record inventory |
| [AI_EDUCATION_RECORDS.md](./AI_EDUCATION_RECORDS.md) | OpenAI FERPA posture |
| [MCP_AGENT_ACCESS.md](./MCP_AGENT_ACCESS.md) | Professor MCP / access-token posture |
| [SUBPROCESSORS.md](./SUBPROCESSORS.md) | Vendor list |
| [FERPA_RIDER.md](./FERPA_RIDER.md) | Institution contract template |
| [BACKUP_RESTORE.md](./BACKUP_RESTORE.md) | Neon restore runbook |
| [CLERK_AUTH_CONTROLS.md](./CLERK_AUTH_CONTROLS.md) | Session / MFA / lockout Dashboard runbook + screenshots |
| [WAF_VERCEL.md](./WAF_VERCEL.md) | Vercel Firewall / WAF enablement |
| [VPAT.md](./VPAT.md) | Self-assessed accessibility ACR (not third-party VPAT) |
| [SSO_FGCU_PLAN.md](./SSO_FGCU_PLAN.md) | Clerk Enterprise SSO → FGCU IdP plan |
| [policies/](./policies/) | SOC 2–style policies |
| [evidence/](./evidence/) | Recurring evidence templates + walkthrough |

Public legal pages (app): `/learning/privacy`, `/learning/terms`.
